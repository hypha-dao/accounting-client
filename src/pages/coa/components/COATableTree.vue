    ...
    async loadAccounts () {
      ...
      const result = this.accounts.accounts.map(account => {
        ...
        return {
          ...
          BTC: this.formatQuantity(BTC ? BTC.value.replace(/[^\d.-]/g, '') : '0'),
          ETH: this.formatQuantity(ETH ? ETH.value.replace(/[^\d.-]/g, '') : '0'),
          EOS: this.formatQuantity(EOS ? EOS.value.replace(/[^\d.-]/g, '') : '0'),
          TLOS: this.formatQuantity(TLOS ? TLOS.value.replace(/[^\d.-]/g, '') : '0'),
          HUSD: this.formatQuantity(HUSD ? HUSD.value.replace(/[^\d.-]/g, '') : '0'),
          SEEDS: this.formatQuantity(SEEDS ? SEEDS.value.replace(/[^\d.-]/g, '') : '0 '),
          HYPHA: this.formatQuantity(HYPHA ? HYPHA.value.replace(/[^\d.-]/g, '') : '0 '),
          parentHash,
          ...usdInfo,
