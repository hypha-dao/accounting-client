...
export default {
  ...
  computed: {
    creditCurrency () {
      ...
      const value = credit.map(c => c.quantity?.value || 0).reduce((previousValue, currentValue) => Number(previousValue + currentValue), 0)
      return `${value} ${currency}`
    },
    debitCurrency () {
      const debit = this.components.filter(c => c.type === 'DEBIT')
      const currency = debit[0]?.currency || ''
      const value = debit.map(c => c.quantity?.value || 0).reduce((previousValue, currentValue) => Number(previousValue + currentValue), 0)
      return `${value} ${currency}`
    }
  },
  methods: {
